...
router.post("/new", async (req, res) => {
  try {
    ...
    let invalids = [];
    if (
      (await Owners.find({ email: req.body.email })).length > 0 ||
      (await Officers.find({ email: req.body.email })).length > 0
    )
      invalids.push("Your email is not available.");
    if (
      (await Owners.find({ serial_number: req.body.serial_number })).length > 0
    )
      invalids.push("Your serial number is not available. you may report it");
