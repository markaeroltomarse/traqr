...
router.post("/newofficer", async (req, res) => {
  try {
    ...
    let invalids = [];
    if (
      (await Owners.find({ email: req.body.data.email })).length > 0 ||
      (await Officers.find({ email: req.body.data.email })).length > 0
    )
      invalids.push("The email is not available.");
...
router.post("/mypreviousscan", async (req, res) => {
  try {
    ...
    violations.forEach(
      vio => (vio.owner = owners.find(o => o._id == vio.ownerID))
    );
